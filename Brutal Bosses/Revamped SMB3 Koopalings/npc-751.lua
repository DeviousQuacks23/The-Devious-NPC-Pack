...
function effectconfig.onTick.TICK_SMB3IGGY(v)
    if v.timer >= 440 then
		v.speedY = 0
	else
		if v.timer == 439 then
...
function iggy.turnToTarget(npc,player)
...
function iggy.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.jumpCount = iggySettings.jumpCounter --or 30
		data.attackTime = iggySettings.attackTimer --or 30
		data.attackDelay = iggySettings.attackDelay --or 10
		data.hp = data.hp or iggySettings.health --or 15
		data.frame = 0
...
function iggy.onNPCHarm (eventObj, killedNPC, killReason, culprit)
