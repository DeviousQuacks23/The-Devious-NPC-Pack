...
npcManager.setNpcSettings(mortonSettings)
...
function effectconfig.onTick.TICK_SMB3MORTON(v)
    if v.timer >= 440 then
		v.speedY = 0
	else
		if v.timer == 439 then
...
function morton.animator(leNPC)
	--data = leNPC.data
	leNPC.data.frame = 0
...
function morton.turnToTarget(npc,player)
...
--Register events
function morton.onInitAPI()
	npcManager.registerEvent(npcID, morton, "onTickNPC")
	--npcManager.registerEvent(npcID, sampleNPC, "onTickEndNPC")
	npcManager.registerEvent(npcID, morton, "onDrawNPC")
	registerEvent(morton, "onNPCHarm")
end

function morton.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.jumpCount = mortonSettings.jumpCounter --or 30
		data.attackTime = mortonSettings.attackTimer --or 30
		data.attackDelay = mortonSettings.attackDelay --or 10
		data.hp = data.hp or mortonSettings.health --or 15
		data.frame = 0
	...
	elseif data.special == 2 then
		...
		if data.attackCount == 0 or data.attackCount == 6 or data.attackCount == 12 then
			...
			projectile.speedX = 3.5 * projectile.direction
...
function morton.onNPCHarm (eventObj, killedNPC, killReason, culprit)
