...
npcManager.setNpcSettings(royshellSettings)
...
--Register events
function royshell.onInitAPI()
	npcManager.registerEvent(npcID, royshell, "onTickEndNPC")
	--npcManager.registerEvent(npcID, royshell, "onTickEndNPC")
	--npcManager.registerEvent(npcID, royshell, "onDrawNPC")
	registerEvent(royshell, "onNPCHarm")
end

function royshell.onTickEndNPC(v)
	...
	if not data.initialized then
		...
		data.special = 0
		data.counter = roySettings.counter --or 0
		data.jumping = false
...
function royshell.onNPCHarm (eventObj, killedNPC, killReason, culprit)
