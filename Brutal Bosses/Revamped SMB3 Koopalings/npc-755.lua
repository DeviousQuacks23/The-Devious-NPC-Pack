...
function ludwig.animator(leNPC)
	--data = leNPC.data
	leNPC.data.frame = 0
...
function ludwig.turnToTarget(npc,player)
...
function ludwig.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.jumpCount = ludwigSettings.jumpCounter --or 30
		data.attackTime = ludwigSettings.attackTimer --or 30
		data.attackDelay = ludwigSettings.attackDelay --or 10
		data.hp = data.hp or ludwigSettings.health --or 15
		data.frame = 0
	...
	if data.special==0 then
		...
		if data.attackTime >= 100 + rng.randomInt(0, 99) and v.speedY == 0 then
			data.special = 1
			p = 0
			data.jumpCount = 0
			data.attackTime = 0
	...
	elseif data.special == 2 then
		v.speedX = 0
		if data.attackCount == 0 or (data.attackCount == 6 and ludwigSettings.spreadShot == false) or (data.attackCount == 12 and ludwigSettings.spreadShot == false) then
			...
			if ludwigSettings.spreadShot == false then
				...
				local projectiler
				if v.direction == -1 then
					projectilec = v.spawn(269, v.x - 18, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
					projectilel = v.spawn(269, v.x - 18, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
					projectiler = v.spawn(269, v.x - 18, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
				else
					projectilec = v.spawn(269, v.x + v.width - 10 + 22, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
					projectilel = v.spawn(269, v.x + v.width - 10 + 22, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
					projectiler = v.spawn(269, v.x + v.width - 10 + 22, v.y+39, v:mem(0x146,FIELD_WORD),false,true)
				end
				...
				projectilec.speedX = -3.5 * math.cos(angle)
				projectilec.speedY = -3.5 * math.sin(angle)
				projectilel.speedX = -3.5 * math.cos(angle-45)
				projectilel.speedY = -3.5 * math.sin(angle-45)
				projectiler.speedX = -3.5 * math.cos(angle+45)
				projectiler.speedY = -3.5 * math.sin(angle+45)
			end
		end
...
function ludwig.onNPCHarm (eventObj, killedNPC, killReason, culprit)
...
function ludwig.onDrawNPC(v)
	if Defines.levelFreeze or Misc.isPaused() then return end
		ludwig.animator(v)
end
