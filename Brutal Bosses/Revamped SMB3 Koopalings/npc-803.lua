...
function effectconfig.onTick.TICK_SMB3WENDY(v)
    if v.timer >= 440 then
		v.speedY = 0
	else
		if v.timer == 439 then
...
local function animation(v)
   ...
   if(v.ai1 == 0) then
		if(v.speedY == 0) then
			if v.speedX == 0 or v.dontMove then
				data.frame = 0;
			else
...
local function ai(v)
	...
	elseif v.ai1 == 2 then
		...
		if v.ai3 == 3 then
			...
			ring.x = ring.x - ((v.direction == 1 and -v.width + 20) or 20)
			ring.y = ring.y - v.height / 4
...
function npc.onTickEndNPC(v)
	...
	data.immunity = data.immunity or 0
...
function npc.onNPCHarm(e, v, r, c)
	...
	if data.hp <= 0 then
	        ...
	        Misc.givePoints(8,vector(v.x + (v.width/2),v.y),true)			
	...
	if r == 1 or r == 2 or r == 8 or r == 3 then
		...
		if r == HARM_TYPE_NPC and type(c) == "NPC" and (c.id ~= 108) then
			c:harm(HARM_TYPE_NPC)
		end
		...
		if r == 3 and c then
			...
			if type(c) == "NPC" and (c.id == 13 or c.id == 108) then
				SFX.play(9)
