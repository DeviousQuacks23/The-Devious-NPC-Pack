...
npcManager.setNpcSettings(roySettings)
...
function effectconfig.onTick.TICK_SMB3ROY(v)
    if v.timer >= 440 then
		v.speedY = 0
	else
		if v.timer == 439 then
...
function roy.turnToTarget(npc,player)
...
function roy.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.jumpCount = roySettings.jumpCounter --or 30
		data.attackTime = roySettings.attackTimer --or 30
		data.attackDelay = roySettings.attackDelay --or 10
		data.hp = data.hp or roySettings.health --or 15
		data.frame = 0
...
function roy.onNPCHarm (eventObj, killedNPC, killReason, culprit)
