--[[
	...
	When the NPC jumps when touching the ground, this file is loaded and is ready to be edited!
	Refer to the end of onTickNPC to see how to disable that jumping when you don't need it.
]]
...
local sampleNPCSettings = {
	...
	--Collision-related
	npcblock = false,
	npcblocktop = true, --Also affects whether thrown NPCs bounce off the NPC.
	playerblock = false,
...
function sampleNPC.onTickEndNPC(v)
	...
	if not data.initialized then
		...
		data.state = 0
		v.speedY = 8
		v.y = Section(v.section).boundary.top - v.height
		data.animTimer = 0
                data.hasThrown = false
	...
	if data.state == 0 then
		v.speedX = v.direction * NPC.config[v.id].speed * 0.5
		v.speedY = math.max(0, v.speedY - 0.08)
		if lunatime.tick() % 4 == 0 then
			local x, y, x2, y2 = v.x + 0.5 * v.width, v.y + 0.1 * v.height, v.x + 0.5 * v.width + 400 * v.direction, v.y + 0.9 * v.height
			if x2 < x then
				x, x2 = x2, x
			end
			for k,p in ipairs(Player.getIntersecting(x, y, x2, y2)) do
        ...
        local frames = NPC.config[npcID].frames / 2

        if not data.hasThrown then
                v.animationFrame = (v.direction == -1 and math.floor(data.animTimer / NPC.config[npcID].framespeed) % frames) or (math.floor(data.animTimer / NPC.config[npcID].framespeed) % frames + NPC.config[npcID].frames)
                for k,p in ipairs(Player.get()) do
                        if Colliders.collide(data.visionCollider[v.direction], p) then
			        local pj = NPC.spawn(757, v.x, v.y + v.height, v.section)
                                pj.y = pj.y - pj.height
                ...
                v.animationFrame = (v.direction == -1 and math.floor(data.animTimer / NPC.config[npcID].framespeed) % frames + frames) or (math.floor(data.animTimer / NPC.config[npcID].framespeed) % frames + frames + NPC.config[npcID].frames)
        end
end
